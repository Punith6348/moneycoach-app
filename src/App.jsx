...
// ─── NET WORTH TAB ────────────────────────────────────────────────────────────
const ASSET_TYPES = [
  { key:"savings",  label:"Savings / FD",   icon:"🏦", color:"#2563EB" },
  { key:"stocks",   label:"Stocks / MF",    icon:"📈", color:"#16A34A" },
  { key:"gold",     label:"Gold",           icon:"🥇", color:"#D97706" },
  { key:"property", label:"Property",       icon:"🏠", color:"#7C3AED" },
  { key:"crypto",   label:"Crypto",         icon:"₿",  color:"#F59E0B" },
  { key:"other",    label:"Other Asset",    icon:"💼", color:"#6B7280" },
];

function NetWorthTab({ assets, loans, totalSavings, onAdd, onUpdate, onDelete, showToast }) {
  ...
  const inp = { width:"100%", padding:"9px 11px", borderRadius:8, border:`1px solid ${C.border}`, fontFamily:"inherit", fontSize:13, background:"#fff", outline:"none", boxSizing:"border-box" };

  const openNew  = () => { setFLabel(""); setFType("savings"); setFValue(""); setFNote(""); setEditId(null); setErr(""); setShowForm(true); };
  const openEdit = (a) => { setFLabel(a.label); setFType(a.type); setFValue(String(a.value)); setFNote(a.note||""); setEditId(a.id); setErr(""); setShowForm(true); };
  const cancel   = () => { setShowForm(false); setEditId(null); setErr(""); };

  const save = () => {
    const v = parseFloat(fValue);
    if (!fLabel.trim()) { setErr("Label is required"); return; }
    if (!v || v <= 0)   { setErr("Enter a valid amount"); return; }
    const data = { label:fLabel.trim(), type:fType, value:Math.round(v), note:fNote.trim() };
    if (editId !== null) { onUpdate(editId, data); showToast("Asset updated ✓"); }
    else                 { onAdd(data);             showToast("Asset added ✓"); }
    cancel();
  };
  ...
  // Compute loan outstanding via the already-available calcLoanTotals
  const loanOutstanding = useMemo(() =>
    loans.reduce((s,l) => {
      // Simple outstanding: use stored principal as fallback (calcLoanTotals imported in LoansTab)
      // Here we approximate: principal is the original amount, reduce by payments made
      const now = new Date();
      ...
      return s + Math.max(0, Math.round(bal));
    }, 0),
    [loans]
  );

  const netWorth    = totalAssets - loanOutstanding;
  const isPositive  = netWorth >= 0;

  // Group assets by type for the breakdown
  ...
  return (
    <div>
      ...
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14}}>
        <div>
          <h2 style={{margin:"0 0 2px", fontSize:17, fontWeight:700, color:C.ink, fontFamily:"Georgia,serif"}}>Net Worth</h2>
          <p style={{margin:0, fontSize:11, color:C.muted}}>Assets − Liabilities · updated manually</p>
        </div>
        {!showForm && (
      ...
      {/* Net Worth Hero */}
      <div style={{
        background:`linear-gradient(135deg, ${isPositive ? "#064E3B" : "#7F1D1D"} 0%, ${isPositive ? "#065F46" : "#991B1B"} 100%)`,
        borderRadius:14, padding:"16px 18px", marginBottom:12,
      }}>
        <p style={{margin:0, fontSize:9, color:"rgba(255,255,255,0.6)", textTransform:"uppercase", letterSpacing:"1.3px", fontWeight:700}}>
          Total Net Worth
        </p>
        <p style={{
          margin:"4px 0 0", fontSize:36, fontWeight:700,
          fontFamily:"Georgia,serif", lineHeight:1,
          color: isPositive ? "#fff" : "#FCA5A5",
        }}>
          {isPositive ? fmt(netWorth) : `−${fmt(Math.abs(netWorth))}`}
        </p>
        <div style={{height:1, background:"rgba(255,255,255,0.12)", margin:"12px 0"}}>  </div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
          <div>
            <p style={{margin:0, fontSize:9, color:"rgba(255,255,255,0.5)", textTransform:"uppercase", letterSpacing:"0.9px"}}>Total Assets</p>
            <p style={{margin:"2px 0 0", fontSize:18, fontWeight:700, fontFamily:"Georgia,serif", color:"#6EE7B7"}}>
              {fmt(totalAssets)}
            </p>
          </div>
          <div>
            <p style={{margin:0, fontSize:9, color:"rgba(255,255,255,0.5)", textTransform:"uppercase", letterSpacing:"0.9px"}}>Loan Liabilities</p>
            <p style={{margin:"2px 0 0", fontSize:18, fontWeight:700, fontFamily:"Georgia,serif", color:"#FCA5A5"}}>
              {fmt(loanOutstanding)}
            </p>
          </div>
        </div>
      ...
      {assets.length > 0 && (
        <div style={{background:"#fff", borderRadius:13, border:`1px solid ${C.border}`, boxShadow:"0 1px 3px rgba(0,0,0,0.05)", overflow:"hidden", marginBottom:12}}>
          <div style={{padding:"10px 14px", borderBottom:`1px solid ${C.bg}`}}>
            <p style={{margin:0, fontSize:13, fontWeight:700, color:C.ink}}>Asset Breakdown</p>
          </div>
          <div style={{padding:"8px 14px 4px"}}>
            {ASSET_TYPES.filter(t => byType[t.key] > 0).map(t => {
              const pct = totalAssets > 0 ? Math.round((byType[t.key] / totalAssets)*100) : 0;
              return (
                <div key={t.key} style={{marginBottom:10}}>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3}}>
                    ...
                    <div style={{display:"flex", gap:8, alignItems:"baseline"}}>
                      <span style={{fontSize:9, color:C.muted, fontWeight:600}}>{pct}%</span>
                      <span style={{fontSize:13, fontWeight:700, color:t.color, fontFamily:"Georgia,serif"}}>{fmt(byType[t.key])}</span>
                    </div>
                  </div>
                  <div style={{height:5, borderRadius:99, background:C.border, overflow:"hidden"}}>
                    <div style={{height:"100%", borderRadius:99, width:`${pct}%`, background:t.color, transition:"width 0.45s"}}/>
                  </div>
                </div>
              );
            })}
      ...
      {/* Loan liabilities summary */}
      {loans.length > 0 && (
        <div style={{background:"#fff", borderRadius:13, border:`1px solid ${C.border}`, boxShadow:"0 1px 3px rgba(0,0,0,0.05)", overflow:"hidden", marginBottom:12}}>
          <div style={{padding:"10px 14px", borderBottom:`1px solid ${C.bg}`}}>
            <p style={{margin:0, fontSize:13, fontWeight:700, color:C.ink}}>Loan Liabilities</p>
            <p style={{margin:"1px 0 0", fontSize:10, color:C.muted}}>Outstanding balances across all loans</p>
          </div>
          <div style={{padding:"4px 0 4px"}}>
            {loans.map((l,i) => {
              ...
              let bal = l.principal||0;
              const emi = l.emi||0;
              for (let j=0; j<elapsed && bal>0; j++) {
                const interest = r>0?bal*r:0;
                const p = Math.min(emi-interest,bal);
                if(p<=0) break;
                bal -= p;
              }
              const outstanding = Math.max(0, Math.round(bal));
              const icons = {"Home Loan":"🏠","Car Loan":"🚗","Personal Loan":"💳","Gold Loan":"🥇","Education Loan":"🎓","Business Loan":"🏢","Two-Wheeler Loan":"🛵","Other":"🏦"};
              return (
                <div key={l.id} style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 14px", borderBottom:i<loans.length-1?`1px solid ${C.bg}`:"none"}}>
                  <div style={{display:"flex", alignItems:"center", gap:8}}>
                    <span style={{fontSize:16}}>{icons[l.name]||"🏦"}</span>
                    <div>
                      <p style={{margin:0, fontSize:12, fontWeight:600, color:C.ink}}>{l.name}</p>
                      <p style={{margin:0, fontSize:10, color:C.muted}}>{l.rate}% · {fmt(emi)}/mo</p>
                    </div>
                  </div>
                  <p style={{margin:0, fontSize:13, fontWeight:700, color:C.red, fontFamily:"Georgia,serif"}}>{fmt(outstanding)}</p>
                </div>
              );
            })}
          </div>
      ...
      {showForm && (
        <div style={{background:"#fff", borderRadius:13, border:`1px solid ${C.border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.06)", padding:16, marginBottom:14}}>
          <p style={{margin:"0 0 14px", fontSize:14, fontWeight:700, color:C.ink}}>
            {editId !== null ? "✏️ Edit Asset" : "💎 Add Asset"}
          </p>

          <p style={{margin:"0 0 4px", fontSize:10, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.9px"}}>Label *</p>
          <input value={fLabel} onChange={e=>{setFLabel(e.target.value);setErr("");}} placeholder="e.g. SBI Savings, Zerodha Portfolio"
            style={{...inp, marginBottom:12}} />

          <p style={{margin:"0 0 6px", fontSize:10, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.9px"}}>Asset Type</p>
          <div style={{display:"flex", flexWrap:"wrap", gap:6, marginBottom:12}}>
            {ASSET_TYPES.map(t=>(
              <button key={t.key} onClick={()=>setFType(t.key)} style={{
                display:"flex", alignItems:"center", gap:4,
                padding:"5px 11px", borderRadius:99, fontSize:11, fontWeight:600,
                cursor:"pointer", fontFamily:"inherit",
                border:`1.5px solid ${fType===t.key ? t.color : C.border}`,
                background:fType===t.key ? `${t.color}15` : "#fff",
                color:fType===t.key ? t.color : C.muted,
              }}>{t.icon} {t.label}</button>
            ))}
          </div>

          <p style={{margin:"0 0 4px", fontSize:10, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.9px"}}>Current Value (₹) *</p>
          <input type="number" value={fValue} onChange={e=>{setFValue(e.target.value);setErr("");}} placeholder="e.g. 250000"
            style={{...inp, fontFamily:"Georgia,serif", fontSize:18, marginBottom:err?4:12}} />
          {err && <p style={{margin:"0 0 10px", fontSize:11, color:C.red, fontWeight:600}}>{err}</p>}
          ...
          <input value={fNote} onChange={e=>setFNote(e.target.value)} placeholder="e.g. Fixed Deposit at HDFC"
          ...
          <div style={{display:"flex", gap:9}}>
            ...
            <button onClick={save}   style={{flex:2, padding:"9px", borderRadius:9, border:"none", background:C.ink, color:"#fff", fontFamily:"inherit", fontSize:13, fontWeight:700, cursor:"pointer"}}>
              {editId !== null ? "Save Changes ✓" : "Add Asset ✓"}
            </button>
          </div>
        </div>
      )}
      ...
      {assets.length > 0 ? (
        <div style={{display:"flex", flexDirection:"column", gap:8}}>
          <p style={{margin:"0 0 6px", fontSize:10, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.8px"}}>Your Assets</p>
          {assets.map(a => {
            const t = ASSET_TYPES.find(x=>x.key===a.type)||ASSET_TYPES[5];
            return (
              <div key={a.id} style={{background:"#fff", borderRadius:11, border:`1px solid ${C.border}`, padding:"11px 14px", boxShadow:"0 1px 2px rgba(0,0,0,0.04)"}}>
                <div style={{display:"flex", alignItems:"center", gap:10}}>
                  <div style={{width:36, height:36, borderRadius:9, flexShrink:0, fontSize:18, background:`${t.color}12`, border:`1px solid ${t.color}25`, display:"flex", alignItems:"center", justifyContent:"center"}}>{t.icon}</div>
                  <div style={{flex:1, minWidth:0}}>
                    <p style={{margin:0, fontSize:13, fontWeight:700, color:C.ink}}>{a.label}</p>
                    <p style={{margin:0, fontSize:10, color:C.muted}}>{t.label}{a.note?` · ${a.note}`:""}</p>
                  </div>
                  <p style={{margin:0, fontSize:15, fontWeight:700, color:t.color, fontFamily:"Georgia,serif", flexShrink:0}}>{fmt(a.value)}</p>
                </div>
                <div style={{display:"flex", gap:6, marginTop:9, paddingTop:9, borderTop:`1px solid ${C.bg}`}}>
                  <button onClick={()=>openEdit(a)} style={{flex:1, padding:"5px", borderRadius:7, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit", border:`1px solid ${C.border}`, background:"#fff", color:C.ink}}>✏ Edit</button>
                  <button onClick={()=>{if(window.confirm(`Delete "${a.label}"?`))onDelete(a.id);}} style={{flex:1, padding:"5px", borderRadius:7, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit", border:"1px solid #FECACA", background:"#FFF1F2", color:C.red}}>🗑 Delete</button>
                </div>
              </div>
            );
          })}
        ...
        <div style={{background:"#EFF6FF", border:"1px solid #BFDBFE", borderRadius:11, padding:"14px 16px", display:"flex", alignItems:"flex-start", gap:10}}>
          <span style={{fontSize:20, flexShrink:0}}>💎</span>
          <p style={{margin:0, fontSize:12, color:C.ink, lineHeight:1.6}}>
            Add your assets — savings, stocks, gold, property. Your Net Worth = Total Assets − Loan Liabilities. This is the number that shows whether you're building wealth over time.
          </p>
        </div>
      )}
    </div>
